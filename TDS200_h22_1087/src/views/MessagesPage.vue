...
import { IonButtons, IonText, IonImg, IonButton, IonItem, IonSpinner, IonList, onIonViewWillEnter, IonContent, IonHeader, IonPage, IonTitle, IonToolbar, RefresherCustomEvent, onIonViewDidLeave } from '@ionic/vue';
...
const fetchSeller = async (id: string) => {
    console.log(chats.value.includes(id));
    ...
    if (response.status === 200 && response.data) {
        ...
        messages.value.forEach( async (message: IMessageResponse) => {
            if(!chats.value.includes(message.sent_to)) {
                //chats.value.push(message.sent_to);
                ...
                //chats.value.push(message.user_created.id);
            }
        });
